...
func (rf *Raft) RequestVote(args *RequestVoteArgs, reply *RequestVoteReply) {
	...
	if rf.state==2&&rf.term<args.Candidate_term&&args.Votemsg==false{
	...
	if args.Candidate_term>=rf.term{
		...
		if args.Append == false {
			...
			//fmt.Printf("Second APPend在服务器端成功！现在编号为%d的raft实例的log是%v\n",rf.me,rf.log)
			}else{
			...
			if args.Prev_log_index != rf.append_try_log_index || args.Prev_log_term != rf.append_try_log_term{
				...
				rf.Log_Term = args.Log_Term
				reply.Append_success = true
				//fmt.Printf("APPend成功，现在编号为%d的raft实例的log是%v\n",rf.me,rf.log)
				}
			}
				...
				//fmt.Printf("将这个编号为%d的raft实例的log_committed只为编号为%d的leader的log_commited %d\n",rf.me,args.From,args.Leader_commited)
		}
...
if rf.leaderID==-1{
	...
	if rf.state==2{
		...
		//fmt.Printf("编号为%d的raft实例是leader，其term是%d, 调用了Start，现在leader的log是%v\n",rf.me,rf.term,rf.log)
		}else{
		...
		case 2: //如果状态是leader
			...
			case <-time.After(time.Duration(50)*time.Millisecond):
				...
				if append_ok>(len(rf.peers)/2){//说明可以commit了
					for cc:=rf.committed_index+1;cc<=rf.Last_log_index;cc++ {
						rf.committed_index=cc
						//fmt.Printf("commit！committed_index是%d\n",cc)
						rf.applych <- ApplyMsg{true, rf.log[rf.committed_index].Log_Command, rf.committed_index}
					}
				}
				rf.mu.Unlock()
