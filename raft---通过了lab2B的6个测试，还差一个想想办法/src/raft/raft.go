...
*/
	...
	if args.Candidate_term>=rf.term{
		...
		if args.Append == false {
			if args.Votemsg == true && rf.voted[args.Candidate_term] == 0&&((args.Newest_log.Log_Term>rf.Last_log_term)||(args.Newest_log.Log_Term==rf.Last_log_term&&args.Last_log_term_lenth>=rf.last_term_log_lenth))  { //合法投票请求
			...
			}else if args.Votemsg == false { //合法的纯heartbeat
				//rf.leaderID=args.From
				//rf.term=args.Candidate_term
			}
		} else { //这条是关于日志的
...
*/
			...
			rf.Last_log_term=args.Candidate_term
...
if args.Commit_MSG==true {
	//if  len(rf.Log_Term)==len(args.Log_Term)&&rf.Log_Term[len(rf.Log_Term)-1]==args.Log_Term[len(args.Log_Term)-1]{
		...
		for cc := rf.committed_index + 1; cc <= rf.Last_log_index; cc++ {
			...
			fmt.Printf("在follower %d 上进行commit，commit_index是%d，commit的内容是%v，commit的term是%d，last_log_term是%d, rf.log是%v\n", rf.me, cc, rf.log[cc].Log_Command, rf.log[cc].Log_Term, rf.Last_log_term, rf.log)
...
if rf.leaderID==-1{
	...
	if rf.state==2{
		...
		fmt.Printf("编号为%d的raft实例是leader，其term是%d, 调用了Start，现在leader的log是%v, term是%d\n",rf.me,rf.term,rf.log,rf.term)
		}else{
		...
		case 2: //如果状态是leader
			...
			if rf.will_commit==true {
				...
				if append_ok>(len(rf.peers)/2){//说明可以commit了
					for cc:=rf.committed_index+1;cc<=rf.Last_log_index;cc++ {
						rf.committed_index=cc
						fmt.Printf("在leader%d进行commit！committed_index是%d，commit的内容是%v，commit的term是%d，last_log_term是%d, rf.log是%v\n",rf.me,cc,rf.log[cc].Log_Command,rf.log[cc].Log_Term,rf.Last_log_term,rf.log)
						rf.applych <- ApplyMsg{true, rf.log[rf.committed_index].Log_Command, rf.committed_index}
					}
					rf.will_commit=true
				}
