		...
		*/
		...
		case 0: //如果状态是follower
			...
			case <-time.After(time.Duration(rand.Intn(666)+300) * time.Millisecond):
						...
						case <-signal:
							if reply.Vote_sent == true {
								...
								if rf.term<reply.Term&&reply.You_are_true{//!!!!!!!!!!!!!!!
									rf.term=reply.Term+6//!!!!!!!!!!!!!!这里多加点，测试程序的服务器最多是五个，这里应该可以证明+的要大于等于5，是个分区相关的不同机器之间的term问题
								}//!!!!!!!!!!!!!!
							}
				...
				//------------------------------rf.mu.Lock()
				if votes >= len(rf.peers)/2 {//这里的这个细节是当收到的投票数量大于等于1/2时，本机才投票给自己，避免给自己的无意义投票，避免可能因为自己的投票额度一直被无意义使用而造成的对其它candidate变成leader的死锁
